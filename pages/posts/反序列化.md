...
摘要：序列化 序列化是将对象的状态信息（属性）转换为可以存储或传输的过程 将对象或者数组转化为可储存/传输的字符串 在php中使用函数serialize来将对象或者数组进行序列化，并返回一个包含字节流的字符串来表示 例子： 所有格式的第一位都是数据类型的英文字母简写， ；是必要的，这才是完整的序列化 数组。
...
stub phar文件标识，格式为`xxx<?php xxx;__HALT_COMLILER(); ?>`(头部信息)
