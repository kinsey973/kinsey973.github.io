...
摘要：Js Js用来负责网页行为 引入方式 内部脚本：将JS代码定义在html界面 javascript必须在\<script\</script之间 在html文档中，可以在任意地方，放置任意数量的\<script，一般放在\<body元素的地步，可改善显示速度 外部脚本：将js代码定义在外部js文件，然。
...
![image-20250815100908024](https://insey.oss-cn-shenzhen.aliyuncs.com/kin/202508151009125.png)
...
-  **高内聚**，指的是一个模块或类的内部功能要尽可能相关、集中，围绕一个明确的目标来组织
- **低耦合**，指的是模块之间的依赖关系要尽量减少和简化

![image-20250819094931621](https://insey.oss-cn-shenzhen.aliyuncs.com/kin/202508190949745.png)

实现分层解耦的思路：将项目中的类交给IOC容器管理(IOC,控制反转)

应用程序运行时需要什么对象，直接依赖容器为其提供(DI，依赖注入)

#### IOC和DI

![image-20250819100043731](https://insey.oss-cn-shenzhen.aliyuncs.com/kin/202508191000790.png)

使用注解@Autowired，程序在运行时会自动找到这个类型的bean对象并赋值给成员变量
