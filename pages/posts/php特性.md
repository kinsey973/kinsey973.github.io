...
摘要：php特性 pregmatch 数组绕过 pregmatch只能处理字符串，当传入的subject是数组时，会返回false payload： num=1 最大回溯次数绕过 PHP 为了防止正则表达式的拒绝服务攻击（reDOS），给 pcre 设定了一个回溯次数上限 pcre.backtrackli。
...
是自逆的，就是hello=atbash(atbash("hello"))，str_rot13是一样的